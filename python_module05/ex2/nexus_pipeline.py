...
class InputStage:
    def process(self, data: Any) -> Dict:
        if not isinstance(data, dict):
            return (
                {"error_message": "Error detected in stage 1:"
                 "Invalid data format"})
        return data
...
class TransformStage:
    def process(self, data: Any) -> Dict:
        if not isinstance(data, dict):
            return (
                {"error_message": "Error detected in stage 2: "
                 "Invalid data format"}
                )
        data["status"] = "ok"
        return data
    ...
    def process(self, data: Any) -> str:
        ...
        if "sensor" in data and "value" in data and "unit" in data:
            ...
            return (f"Processed {data['sensor']} reading: "
                    f"{data['value']}°{data['unit']} (Status: {status})")
        if "user" in data and "action" in data:
            timestamp = "unknown time"
            if "timestamp" in data:
                timestamp = data["timestamp"]
            return (f"User {data['user']} performed "
                    f"{data['action']} at {timestamp}")
...
def main() -> None:
    try:
        ...
        print(
            f"{json_pipeline.pipeline_id} -> "
            f"{csv_pipeline.pipeline_id} -> {stream_pipeline.pipeline_id}"
        )
